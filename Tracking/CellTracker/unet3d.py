...
def unet3_prediction(input, model, shrink=(24, 24, 2)):
    # predict cell/non-cell regions by applying unet_model on each sub-image. 
    # to improve the reliability, the predictions of surrounding parts of each sub-image 
    # was discarded according to the shrink parameters.
    
    # input image is expanded outside by "reflection" to predict the voxels near borders.
    ...
    new_siz1 = int(math.ceil(input.shape[1]*1.0/siz1))*siz1
    new_siz2 = int(math.ceil(input.shape[2]*1.0/siz2))*siz2
    new_siz3 = int(math.ceil(input.shape[3]*1.0/siz3))*siz3
    
    pad1a, pad2a, pad3a = shrink[0], shrink[1], shrink[2]
    pad1b, pad2b, pad3b = new_siz1+pad1a-input.shape[1], \
        new_siz2+pad2a-input.shape[2], new_siz3+pad3a-input.shape[3]

    input_pad = np.pad(input[0, :, :, :, 0], ((pad1a, pad1b), (pad2a, pad2b),
                       (pad3a, pad3b)), 'reflect')
    ...
    out_img = img[:, 0:input.shape[1], 0:input.shape[2], 0:input.shape[3], :]
    return out_img
