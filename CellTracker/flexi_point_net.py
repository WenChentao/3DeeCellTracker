...
from tensorflow.keras.layers import Dense, BatchNormalization, LeakyReLU, Concatenate, Conv2D, Flatten
...
class TrainFPM:
    def __init__(self, model_name: str, points1_path: str = None, segmentation1_path: str = None, voxel_size: tuple = (1, 1, 1),
        ...
        self.model = FlexiblePointMatcher()
       ...
       # Load the points set
        ...
        self.points_generator = DataGeneratorFPN(self.points_t1, num_rotations=10, num_deformations=10, movement_factor=move_factor)
...
class FlexiblePointMatcher(Model):
    """
    ...
    def up_cnn():
        return tf.keras.Sequential([
            Conv2D(64, kernel_size=(K_NEIGHBORS+2, 1), input_shape=(K_NEIGHBORS+2, NUM_FEATURES, 2), padding='valid'),
            BatchNormalization(),
            LeakyReLU(),
    ...
    def call(self, points_xy):
        expanded_feature = self.cnn(points_xy)
