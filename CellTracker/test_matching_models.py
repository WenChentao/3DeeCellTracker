...
def rotation_align_by_fpm(fpm_models_rot, coords_norm_t1, coords_norm_t2, similarity_threshold=0.4,
                          threshold_mdist=3 ** 2):
    ...
    return _transform_by_control_points(
        coords_norm_t1, coords_norm_t2, "euclidean",
...
def _transform_by_control_points(coords_norm_t1, coords_norm_t2, method_transform: str, threshold_mdist: float,
    ...
    align_func = local_align_by_control_points if local_cal and pairs_px2.shape[
        0] > N_NEIGHBOR_LOCAL_ALIGNMENT else align_by_control_points
...
def _match_pure_fpm(coords_norm_t1: ndarray, coords_norm_t2: ndarray, fpm_models, similarity_threshold: float,
                    prob_mxn_initial: ndarray = None):
    ...
    if prob_mxn_initial is None:
        ...
        initial_matching = initial_matching_fpm_local_search(fpm_models[0], coords_norm_t1, coords_norm_t2, K_POINTS,
                                                             prob_mxn_initial)
...
def match_by_prgls(pairs_px2, coords_norm_t1, coords_norm_t2, similarity_threshold_final=0.3):
...
def prgls_with_two_ref(normalized_prob_mxn, ptrs_tgt_mx3: ndarray, ptrs_ref_nx3: ndarray, tracked_ref_lx3: ndarray,
    ...
    for iteration in range(1, max_iteration):
        ...
        posterior_mxn = estimate_posterior(
            normalized_prob_mxn, sigma_square,
            predicted_coord_ref_nx3, ptrs_tgt_mx3,
            ratio_outliers)
...
def gaussian_kernel(ptrs_ref_nx3: np.ndarray, ptrs_tgt_mx3: np.ndarray, sigma_square: float) -> np.ndarray:
    # 计算点对之间的欧几里得距离平方和 (矢量化，无需扩展维度)
    dist_square_sum_mxn = (
            np.sum(ptrs_ref_nx3 ** 2, axis=1)[:, None]  # nx1
            + np.sum(ptrs_tgt_mx3 ** 2, axis=1)[None, :]  # 1xm
            - 2 * np.dot(ptrs_ref_nx3, ptrs_tgt_mx3.T)  # nxm
    ).T
    # 计算高斯核结果
...
def solve_movements_ref(initial_sigma_square, lambda_, posterior_mxn, ptrs_ref_nx3, ptrs_tgt_mx3,
    ...
    posterior_sum_n = np.sum(posterior_mxn, axis=0)
    coefficient_nxn = scaling_factors_nxn * posterior_sum_n + lambda_ * initial_sigma_square * np.identity(n)
    dependent_3xn = np.dot(ptrs_tgt_mx3.T, posterior_mxn) - ptrs_ref_nx3.T * posterior_sum_n
