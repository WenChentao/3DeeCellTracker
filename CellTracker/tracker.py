...
def save_automatic_segmentation(labels_xyz: ndarray, folder_path, use_8_bit: bool):
    ...
    for z in range(1, labels_xyz.shape[2] + 1):
        ...
        Image.fromarray(img2d).save(os.path.join(folder_path, "auto_vol1", "auto_vol1_z%05i.tif"%z))
...
def save_img3ts(z_range, img, path, t, use_8_bit: bool=True):
    ...
    path : str
        ...
        It should use formatted string to indicate volume number and then layer number, e.g. "xxx_t%05d_z%05i.tif"
    t : int
...
class Segmentation:
    ...
    def _predict_cellregions(self, image_raw, vol):
        ...
        """
        try:
            image_cell_bg = np.load(self.paths.unet_cache + "t%05i.npy" % vol, allow_pickle=True)
        except OSError:
    ...
    def _save_unet_regions(self, image_raw, vol):
        ...
        np.save(self.paths.unet_cache + "t%05i.npy" % vol, np.array(image_cell_bg, dtype="float16"))
        return image_cell_bg
...
class Tracker(Segmentation, Draw):
    ...
    def interpolate_seg(self):
        ...
        save_img3ts(range(0, self.z_siz), self.segmentation_manual_relabels,
                    self.paths.track_results + "track_results_t%05i_z%05i.tif", t=1, use_8_bit=self.use_8_bit)
    ...
    def replay_track_animation(self, from_volume=2):
        ...
        for volume in range(from_volume, self.volume_num + 1):
            try:
                im = mgimg.imread(self.paths.anim + "track_anim_t%05i.png" % volume)
    ...
    def track_one_vol(self, target_volume, fig, axc6, method="min_size"):
        ...
        if target_volume in self.miss_frame:
            save_img3ts(range(0, self.z_siz), self.tracked_labels,
                        self.paths.track_results + "track_results_t%05i_z%05i.tif", target_volume, self.use_8_bit)
        ...
        fig.canvas.draw()
        plt.savefig(self.paths.anim + "track_anim_t%05i.png" % target_volume, bbox_inches='tight')
